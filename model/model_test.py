...
FEATURE_DIM = 768 # vit_base 768 vit_large 1024
...
LOCAL_CHECKPOINT_PATH = "/data/truenas_B2/yyi/xray_dino_512/eval/training_71999/teacher_checkpoint.pth" # 替换为您的本地 .pth 文件路径
CFG_PATH = "/data/truenas_B2/yyi/train/pretrain_2/dinov3_vitb16_pretrain.yaml"
LOAD_LOCAL_CHECKPOINT = False # 是否加载本地检查点
if LOAD_LOCAL_CHECKPOINT:
    TEST_NAME = "xrayDinov3_71999"
else:
...
def train_multi_task_classifier(logger: logging.Logger):
    ...
    train_df, temp_df = train_test_split(
        df, test_size=0.3, random_state=42, stratify=df['label']  # stratify 保持类别分布
    )
    ...
    # 初始化模型、损失函数和优化器
    model = DinoV3MultiTaskClassifier(MODEL_TYPE, num_classes_dict).to(DEVICE)
    # 创建任务特定的加权损失函数字典
    criterion_dict = {}
    for task in LABEL_COLUMNS:
